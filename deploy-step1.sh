...
if [[ -z ${OPENSHIFT_INSTALL_RELEASE+x} ]]; then
    ...
    OKD_DOWNLOAD_URL="$(curl -s https://api.github.com/repos/okd-project/okd/releases | jq -r '.[].assets[] | select(.name | contains("openshift-install-mac-4.12.0-0.okd-2023-04-16-041331")) | .browser_download_url')"
fi
